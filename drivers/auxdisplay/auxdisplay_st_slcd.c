...
static void auxdisplay_st_slcd_display_on(const struct device *dev)
{
...
static void auxdisplay_st_slcd_display_off(const struct device *dev)
{
	const struct auxdisplay_st_slcd_config *config = dev->config;
					     ...
					     uint8_t bar_value, uint32_t *pattern)
{
	...
	case SLCD_ASCII_TRIPLE_DOT:
		map = ST_SLCD_GH08172T_TRIPLE_DOT;
		break;
	case '0':
	case '1':
	case '2':
	...
	if (position < GH08172T_DIGIT_POSITION_5) {
		if (point == GH08172T_POINT_SINGLE || point == ST_SLCD_GH08172T_TRIPLE_DOT) {
			map |= SLCD_C_SINGLE_DOT_MAP;
		}
		if (point == GH08172T_POINT_DOUBLE || point == ST_SLCD_GH08172T_TRIPLE_DOT) {
	...
	if (position < GH08172T_DIGIT_POSITION_6) {
		...
		case SLCD_ASCII_DOUBLE_DOT:
			*point = GH08172T_POINT_DOUBLE;
			...
			break;
		case SLCD_ASCII_TRIPLE_DOT:
			*point = ST_SLCD_GH08172T_TRIPLE_DOT;
			(*i)++;
			break;
		default:
			break;
