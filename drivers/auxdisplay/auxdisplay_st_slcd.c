...
static void st_slcd_prepare_custom_character(const struct device *dev, uint8_t ascii_char,
					     int position)
{
	struct auxdisplay_st_slcd_data *data = dev->data;

	if (ascii_char >= data->custom_character_slot_used) {
		LOG_WRN("Custom character not supported (0x%X >= 0x%X)", ascii_char,
			data->custom_character_slot_used);
		return;
	}

	st_slcd_write_pattern_to_buffer(dev, data->custom_character_patterns[ascii_char], position);
}
...
static int auxdisplay_st_slcd_write(const struct device *dev, const uint8_t *text, uint16_t len)
{
	...
	for (int i = 0, position = data->cursor_y * cols + data->cursor_x;
		...
		if (one_char < data->custom_character_slot_used) {
			st_slcd_prepare_custom_character(dev, one_char, position);
		} else {
